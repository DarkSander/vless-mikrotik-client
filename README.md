...
ещё не поднятый туннель (см. «Резолв адреса сервера»). **Не проверено**:
сама сборка образа (на машине разработки нет Docker), поведение на железе
RouterOS/в функции Container и сквозной туннель целиком — устройства
MikroTik под рукой нет.
...
|---|---|---|
| `SINGBOX_ARCH` | `arm64` | `arm64`, `amd64` или `armv7` — под процессор роутера. RouterOS Container работает только на ARM, ARM64 и x86_64 |
...
Суммарно (расчёт сложением, не проверен реальной сборкой — на машине
разработки нет Docker): было около **82 МиБ**, стало около **40 МиБ**, а с
`COMPRESS=true` — около **12 МиБ**. Для сравнения, распространённый образ
`wiktorbgu/vless-sing-box-tunnel-mikrotik` весит 15.7 МБ в сжатом виде;
наш сжатый слой sing-box — 12.5 МиБ.

Про `COMPRESS=true`: UPX ужимает файл на диске втрое, но при старте
бинарник распаковывается в память целиком, то есть экономится место на
накопителе роутера, а не оперативная память. На устройствах с 128–256 МБ
ОЗУ это стоит включать осознанно; по умолчанию выключено.
