...
DNS-запросы клиентов перехватываются правилом `hijack-dns`. В sing-box
1.11+ условие `protocol: dns` срабатывает только после действия `sniff`,
поэтому оно стоит первым правилом. Без него запрос к DNS-серверу из
`/etc/resolv.conf` уходил в туннель как обычный UDP и не резолвился
(проверено в контейнере: с `sniff` в логе появляется `inbound DNS packet`
и имена разрешаются).
